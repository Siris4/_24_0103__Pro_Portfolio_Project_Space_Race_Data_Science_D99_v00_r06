...
def scrape_page():
    ...
    for i, launch in enumerate(launch_blocks):
        try:
            ...
            try:
                details_url = launch.find_element(By.XPATH, ".//a[contains(@href, '/launches/details')]").get_attribute(
                    'href')
            except NoSuchElementException:
                details_url = "Unknown"
            ...
            try:
                watch_url = launch.find_element(By.XPATH,
                                                ".//a[contains(@href, 'youtube') or contains(@href, 'spacex')]").get_attribute(
                    'href')
            except NoSuchElementException:
                watch_url = "Unknown"
...
def go_to_next_page():
    try:
        next_button = driver.find_element(By.XPATH,
                                          "//a[@class='mdc-button mdc-button--raised' and contains(., 'next')]")
        next_button.click()
...
for page in range(1, 3):
    ...
    if page < 2:  # Click the "Next" button only for the first page
